...
function processPost(post) {
	...
	let stringDate = postDate.getFullYear() + '-' + getPaddedMonthNumber(postDate.getMonth() + 1) + '-' + getPaddedDayNumber(postDate.getDate())

	var fullPath = `out/${stringDate}-${slug}`;
	...
	fs.mkdir(fullPath, function() {
		...
		fs.writeFile(fullPath + "/" + stringDate + '-' + slug + '.md', header + markdown, function(err) {
